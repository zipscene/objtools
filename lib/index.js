...
const getDuplicateKeys = values => getDuplicates(getKeys(values));

const objectsIteratee = (memo, val, index) =>
	_.extend(memo, { [index]: isScalar(val) ? val : null });
function diffObjects(...objects) {
	let diffKeys = _.reject(getKeys(objects), isEveryValueAtKeyEqual(objects));
	let result = _.any(objects, isScalarAndDefined)
		? _.reduce(objects, objectsIteratee, [])
		: {};
	const diffKeysReducer = (result, key) => {
		let diffValues = _.map(objects, obj => _.has(obj, key) ? obj[key] : null);
		if (getDuplicateKeys(diffValues).length) {
			result[key] = _.any(diffValues, isScalar) ? diffValues : {};
			let everyResultIsCollectionOrNull = _.every(result[key], isCollectionOrNull);
			let hasDuplicateKeys = !!getDuplicateKeys(result[key]).length;
			let isRelevant = !everyResultIsCollectionOrNull && !hasDuplicateKeys;

			if (isRelevant) {
				_.extend(result[key], exports.diffObjects(...diffValues));
			} else {
				result[key] = exports.diffObjects(...diffValues);
			}
		} else {
	...
	return diffKeys.reduce(diffKeysReducer, result);
}
