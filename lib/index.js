...
function merge(object, ...sources) {
	let customizer, lastSource = _.last(sources);
	if (sources.length > 1 && _.isFunction(_.last(sources))) {
		customizer = lastSource;
		sources = sources.slice(0, -1);
	}
	// check if merge is being used w/ reduce or similar
	let isIterator = sources.length > 1 && _.isArray(lastSource) && _.contains(lastSource, _.head(sources));
	if (isIterator) {
		sources = sources.slice(0, 1);
	}
	_.each(sources, source => baseMerge(object, source, customizer));
	return object;
}
exports.merge = merge;
	...
	if (!isScalar(object)) {
		for (let key of _.keys(source)) {
			...
			if (isScalar(srcValue)) {
				...
				let hasValue = _.isArray(source) || typeof result !== 'undefined';
				let isNewValue = result !== value && !(isNaN(result) && isNaN(value));
				if (hasValue && (isCommon || isNewValue)) { object[key] = result; }
			} else {
 ...
 */
const isValueAtKeyEqual = (key, objects) => obj => (obj && objects[0] && obj[key] === objects[0][key]);
const isEveryValueAtKeyEqual = objects => key => _.every(objects, isValueAtKeyEqual(key, objects));
const isCollectionOrNull = val => (!isScalar(val) || val === null);
const isScalarAndDefined = obj => (isScalar(obj) && obj !== undefined && obj !== null);

const getKeys = objects => _.flatten(_.map(_.reject(objects, isScalar), _.keys));
const getDiffKeys = objects => _.reject(getKeys(objects), isEveryValueAtKeyEqual(objects));
const getOverlappingCollectionKeys = diffValues => {
	const collections = _.reject(diffValues, isScalar);
	const collectionKeys = _.flatten(_.map(collections, _.keys));
	let result =  _.keys(_.omit(_.countBy(collectionKeys), (count => count < 2)));
	return result;
};
const getValAtKeyOrNull = key => obj => _.has(obj, key)
	? obj[key]
	: null;

const hasScalars = diffValues => _.any(diffValues, isScalar);
const hasOverlappingCollectionKeys = diffValues => !!getOverlappingCollectionKeys(diffValues).length;

const objectsIteratee = (memo, val, index) => _.extend(memo, { [index]: isScalar(val) ? val : null });
const handleObjects = objects => _.any(objects, isScalarAndDefined)
	? _.reduce(objects, objectsIteratee, [])
	: {};

const scalarsIteratee = (diff, diffValue) => (diff.push(diffValue), diff);
const handleScalars = diffValues => _.reduce(diffValues, scalarsIteratee, []);

const handleOverlappingCollectionKeys = diffValues => {
	let result = hasScalars(diffValues) ? handleScalars(diffValues) : {};
	let isRelevant = !(_.every(result, isCollectionOrNull) ||
			hasOverlappingCollectionKeys(result));
	return _.extend(isRelevant ? result : {}, exports.diffObjects(...diffValues));
};
const handleDiffValues = diffValues => {
	const overlappingCollectionKeys = getOverlappingCollectionKeys(diffValues);
	if (overlappingCollectionKeys.length) return handleOverlappingCollectionKeys(diffValues);
	else return diffValues.every(_.isNull) ? null : diffValues;
};

function diffObjects(...objects) {
	let diffKeys = getDiffKeys(objects);
	let result = handleObjects(objects);
	for (let key of diffKeys) {
		let diffValues = _.map(objects, getValAtKeyOrNull(key));
		result[key] = handleDiffValues(diffValues);
	}
	return result;
}
