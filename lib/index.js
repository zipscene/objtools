...
/**
 * Convenience function to for:
 * new ObjectMask(mask).filterObject(obj, maskedOutHook)
 *
 * @method filterObjByMask
 * @static
 * @param {Object} obj - The object to filter
 * @param {Object} mask - The mask spec to filter by
 * @param {Function} [maskedOutHook] - Hook to call for fields disallowed by the mask
 * @return {Object} - Object after masked out fields are removed
 */
function filterObjByMask(obj, mask, maskedOutHook) {
	return new ObjectMask(mask).filterObject(obj, maskedOutHook);
}
 ...
 * new ObjectMask(mask).getSubMask(path)
 *
 * @method getSubMask
 * @static
 * @param {Object} mask
...
function getSubMask(mask, path) {
	return new ObjectMask(mask).getSubMask(path);
}
exports.getSubMask = getSubMask;

/**
 * Convenience function for new ObjectMask(mask).checkPath(path)
 * 
...
function checkMaskPath(mask, path) {
	return new ObjectMask(mask).checkPath(path);
}
