...
class ObjectMask {
	 ...
	 * Array-type wildcards on path are converted to underscores.
	 *
	 * @method removeField
	 ...
	 */
	removeField(path) {
		if (path === '_' || path === '0' || path.slice(-2) === '._' || path.slice(-2) === '.0') {
			...
			let subpathsToCurrentSubmask = [];
			while (submask) {
				let wildcardSubmask = submask._ || submask[0];
				let nextSubpath = subpaths.shift();
				let nextSubmask = submask[nextSubpath];
				subpathsToCurrentSubmask.push(nextSubpath);
				if (typeof nextSubmask === 'object') {
					submask = nextSubmask;
				} else if (wildcardSubmask && !nextSubmask) {
					if (submask instanceof Array) {
						let pathToCurrentSubmask = subpathsToCurrentSubmask.slice(0, -1).join('.');
						objtools.setPath(this.mask, pathToCurrentSubmask, { _: wildcardSubmask });
					}
	...
	static createMaskFromFieldList(fields) {
		...
		for (let index = 0; index < fields.length; index++) {
			objtools.setPath(ret, fields[index], true);
		}
...
function underscorizeArrays(mask) {
	for (let subpath of _.keys(mask)) {
