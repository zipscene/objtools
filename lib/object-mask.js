...
class ObjectMask {
	...
	static subtractMasks(min, sub) {
		return new ObjectMask(min instanceof ObjectMask ? min.mask : min).subtractMask(sub);
	}
...
function subtract(a, b) {
	...
	if (a === true) {
		...
		return a;
	} else if (!a || b === true || _.isEqual(a, b)) {
		return false;
	}

	let key;
	if ('_' in a && '_' in b) {	// both have _
		a._ = subtract(a._, b._);
		for (key in b) {
			if (key !== '_') {
	...
	// TODO: pull this cleanup out to separate helper and call on instantiation
	// only use falses to override wildcards
	if (!a._) {
		for (key in a) {
			if (a[key] === false) delete a[key];
		}
	}
	// empty masks aren't helpful
